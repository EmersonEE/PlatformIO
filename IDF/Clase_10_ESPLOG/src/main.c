...
void app_main() {
  ...
  gpio_config_t io_conf = {.pin_bit_mask = (1ULL << GPIO_NUM_12),
                           .mode = GPIO_MODE_OUTPUT,
                           .pull_up_en = GPIO_PULLUP_DISABLE,
                           .pull_down_en = GPIO_PULLDOWN_DISABLE,
                           .intr_type = GPIO_INTR_DISABLE};
  esp_err_t error = gpio_config(&io_conf);
  ESP_LOGI(TAG, "INciando APlicacion del ESP2");
  ...
  while (true) {
    ...
    if (error != ESP_OK) {
      ESP_LOGE(TAG, "Error al establecer nivel alto: %s",
    ...
    if (error != ESP_OK) {
      ESP_LOGE(TAG, "Error al establecer nivel bajo: %s",
               esp_err_to_name(error));
      return;
    }
