...
/** PCF8574 instance */
PCF8574 expander;
/** This function will be called each time the state of a pin of the PCF8574
 * change */
void ISRgateway() { expander.checkForInterrupt(); }

/** This function will be called each time the button on pin 3 is pressed
...
void setup() {
  ...
  /* Enable PCF8574 interrupts, use pin D8 as "INT" pin and ISRgateway() as
   * callback function */
...
void loop() {
  ...
  Serial.println(expander.digitalRead(3) ? "HIGH"
                                         : "LOW"); // Print button pin state
  Serial.println(expander.read(), DEC); // Read the whole pins input register
  expander.attachInterrupt(3, ISRdemo,
                           FALLING); // Re-enable interrupt on button pin
